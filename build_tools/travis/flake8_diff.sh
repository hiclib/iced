...
# We ignore files from XXX/externals. Unfortunately there is no
...
# ':!XXX/externals' but it is only available on git 1.9 and Travis
# uses git 1.8.
# We need the following command to exit with 0 hence the echo in case
# there is no match
MODIFIED_FILES="$(git diff --name-only $COMMIT_RANGE | grep -v 'XXX/externals' \
                    || echo "no_match")"
